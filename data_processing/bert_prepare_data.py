...
class bertPrepareData(BaseDataPreparing):
    def __init__(self,vocab_file,slot_file,bert_file,max_length,gen_new_data=False,is_inference=False):
        self.bert_file = bert_file
        self.max_length = max_length
        super(bertPrepareData,self).__init__(vocab_file,slot_file,pretrained_embedding_file=None,word_embedding_file=None,load_w2v_embedding=False,gen_new_data=gen_new_data,is_inference=is_inference)
    ...
    def tranform_singlg_data_example(self,text):
        ...
        if self.is_inference:
            word_list.extend([w for w in text])
        else:
